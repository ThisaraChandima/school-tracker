    ...
    <div style={{ minHeight: '100vh', background: '#f7f8fa', fontFamily: "'Plus Jakarta Sans',sans-serif", color: '#111827' }}>
      ...
      <section style={{ background: '#0f1f3d', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '96px 24px 64px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontSize: 'clamp(36px,6.5vw,70px)', fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.06, color: '#fff', margin: '0 0 18px', opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(16px)', transition: 'all .65s ease .15s' }}>
            Track Issues.<br />
            <span style={{ color: '#60a5fa' }}>Drive Change.</span>
          </h1>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap', opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(10px)', transition: 'all .65s ease .35s' }}>
            <Link href="/tracker" style={{ padding: '12px 26px', borderRadius: 9, fontSize: 15, fontWeight: 700, background: '#1a56db', color: '#fff', textDecoration: 'none', transition: 'opacity .18s' }}
              onMouseEnter={e => e.currentTarget.style.opacity = '.88'} onMouseLeave={e => e.currentTarget.style.opacity = '1'}>
              Browse Issues →
            </Link>
            <Link href="/login" style={{ padding: '12px 26px', borderRadius: 9, fontSize: 15, fontWeight: 700, background: 'rgba(255,255,255,0.08)', color: '#fff', textDecoration: 'none', border: '1.5px solid rgba(255,255,255,0.14)', transition: 'background .18s' }}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: 60, paddingTop: 32, borderTop: '1px solid rgba(255,255,255,0.08)', flexWrap: 'wrap', opacity: vis ? 1 : 0, transition: 'all .65s ease .45s' }}>
            {[
              ...
              { label: 'Issues Logged', value: stats?.total ?? 0, color: '#93c5fd' },
              { label: 'Open',          value: stats?.open ?? 0,  color: '#fca5a5' },
              { label: 'Resolved',      value: stats?.done ?? 0,  color: '#86efac' },
            ].map((s, i) => (
              <div key={i} style={{ flex: '1 0 80px', textAlign: 'center', padding: '0 16px', borderRight: i < 3 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
                <div style={{ fontSize: 'clamp(26px,3.5vw,38px)', fontWeight: 800, color: s.color, lineHeight: 1, letterSpacing: '-0.04em' }}>
                  <Num value={s.value} />
                </div>
      ...
      <section style={{ padding: '72px 24px', maxWidth: 880, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 10 }}>
          {[
            { icon:'🏫', label:'Total Schools',    value:134,                color:'' },
            { icon:'📋', label:'Issues Logged',    value:stats?.total??0,    color:'#1a56db' },
            { icon:'⚠️', label:'Open Issues',      value:stats?.open??0,     color:'#dc2626' },
            { icon:'✅', label:'Resolved',         value:stats?.done??0,     color:'#16a34a' },
            { icon:'🏛️', label:'Schools Tracked',  value:stats?.schoolsWithIssues??0, color:'' },
            { icon:'🌟', label:'Fully Cleared',    value:stats?.schoolsAllDone??0,    color:'#d97706' },
          ].map((s, i) => (
            <Reveal key={i} delay={i * 0.05}>
              <div className="card-hover" style={{ background: '#fff', border: '1.5px solid #e5e7eb', borderRadius: 14, padding: '18px 14px', textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: '-0.03em', color: s.color || '#111827', marginBottom: 4 }}>
                  <Num value={s.value} />
                </div>
      ...
      <section style={{ background: '#fff', padding: '64px 24px', borderTop: '1.5px solid #e5e7eb', borderBottom: '1.5px solid #e5e7eb' }}>
        <div style={{ maxWidth: 880, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 14 }}>
            {[
              { icon:'🔍', title:'Smart Search',       desc:'Find schools instantly by name or area in English and Sinhala. Works with type and census ID too.' },
              ...
              { icon:'✅', title:'Track Resolutions',  desc:'Mark issues resolved, reopen if needed. Full date history for every change.' },
              { icon:'📊', title:'Live Dashboard',     desc:'Real-time zone-wide stats update as issues are logged and resolved across all schools.' },
              { icon:'🔐', title:'Role-based Access',  desc:'Admin manages everything. Individual schools log in to manage only their own issues.' },
              { icon:'📱', title:'Works Everywhere',   desc:'Fully responsive — optimised for phones, tablets, and desktop computers.' },
            ].map((f, i) => (
        ...
        <Reveal>
          <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center', background: '#0f1f3d', borderRadius: 24, padding: 'clamp(40px,5vw,60px) 32px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 15, marginBottom: 30, maxWidth: 380, margin: '0 auto 30px', lineHeight: 1.65 }}>
                Sign in as admin to track and resolve issues across all 134 schools.
              </p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                <Link href="/login" style={{ padding: '12px 26px', borderRadius: 9, fontSize: 15, fontWeight: 700, background: '#1a56db', color: '#fff', textDecoration: 'none' }}>
                  Admin Sign In →
                </Link>
                <Link href="/tracker" style={{ padding: '12px 26px', borderRadius: 9, fontSize: 15, fontWeight: 700, background: 'rgba(255,255,255,0.08)', color: '#fff', textDecoration: 'none', border: '1.5px solid rgba(255,255,255,0.14)' }}>
                  Public Tracker
                </Link>
              </div>
            </div>
